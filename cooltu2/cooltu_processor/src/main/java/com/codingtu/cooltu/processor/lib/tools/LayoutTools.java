...
public class LayoutTools {
    ...
    public static List<ViewInfo> convert(String layoutName) {
        ...
        return Ts.ts(viewInfoMap).toList().convert(new BaseTs.Convert<List<ViewInfo>, ViewInfo>() {
            @Override
            public ViewInfo convert(int index, List<ViewInfo> viewInfos) {
    ...
    private static void dealViewInfo(ListValueMap<String, ViewInfo> viewInfoMap, ViewInfo viewInfo) {
        if (viewInfo != null) {
            if (StringTool.isNotBlank(viewInfo.id)) {
                viewInfoMap.get(viewInfo.fieldName).add(viewInfo);
            }
            Ts.ls(viewInfo.childs, new BaseTs.EachTs<ViewInfo>() {
                @Override
    ...
    private static void dealViewinfo(ListValueMap<String, ViewInfo> viewInfoMap) {
        ...
        for (String key : keys) {
            List<ViewInfo> viewInfos = viewInfoMap.get(key);
    ...
    private static void changeFieldName(ViewInfo info) {
